...
{    
	...
	node_idx = *ADDR_rev_pos(H,node_idx);//finds the index of key pos that corresponds to the node
	
	size_t* node_key = ADDR_key_pos(H,node_idx);
	
	//if node does not belong to H or *value is > *node
	//the operation cannot be done(we cannot decrease node to value), return NULL
	if(!VALID_NODE(H,node_idx) || !(H->leq(value, ADDR(H,*node_key)))){//again a comparison by address
			
		
		return NULL; //
	}

	memcpy(ADDR(H,*node_key),value, H->key_size);//change the value of the node
	...
	while((node_idx != 0) && (!H->leq(ADDR(H,*parent),ADDR(H,*node_key)))){
		...
		//moving up the check
		node_idx = parent_idx;
		node_key =parent;//we need the adress in order to compare under leq
...
const void *insert_value(binheap_type *H, const void *value)
{
	...
	*new_node = H->num_of_elem_A;
	...
	//set rev_pos to the correct value
	*ADDR_rev_pos(H,H->num_of_elem_A) = H->num_of_elem;//this is the new index on rev_pos
